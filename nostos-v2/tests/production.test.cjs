...
test('production analytics: consent gate, correct ids, single PageView and checkout at 39 EUR',()=>{
 const r=tracking(undefined,false);assert.equal(r.scripts.length,0);r.handlers.yes();assert.equal(r.scripts.length,3);r.handlers.buy();assert.equal(r.events.filter(e=>e[1]==='PageView').length,1);assert.equal(r.events.find(e=>e[1]==='InitiateCheckout')[2].value,39);
});
